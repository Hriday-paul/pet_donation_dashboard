...
import { useAdminShelterStatsQuery } from "@/redux/api/dashboard.api";
...
const ShelterOverviewChart = () => {
  ...
  const { isError, data } = useAdminShelterStatsQuery({ "year": selectedYear }, { refetchOnMountOrArgChange: true });
  ...
  const realdata: { name: string, shelters: number }[] = [];

  const monthlyData = data?.data?.monthlyJoinedShelter;

  for (let i in monthlyData) {
    const month = i as keyof IMonth;
    realdata.push({ name: month.substring(0, 3), shelters: monthlyData[month] })
  }
