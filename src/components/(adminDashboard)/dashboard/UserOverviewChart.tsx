...
import { useAdminUserStatsQuery } from "@/redux/api/dashboard.api";
...
const UserOverviewChart = () => {
  ...
  const { isError, data } = useAdminUserStatsQuery({ "year": selectedYear }, { refetchOnMountOrArgChange: true });
  ...
  const realdata: { name: string, users: number }[] = [];

  const monthlyData = data?.data?.monthlyJoinedUser;

  for (let i in monthlyData) {
    const month = i as keyof IMonth;
    realdata.push({ name: month.substring(0, 3), users: monthlyData[month] })
  }
  ...
  return (
    <div className="bg-secondary-color border border-stroke rounded-3xl p-8 ">
      <div className="text-text-color flex lg:flex-wrap xl:flex-nowrap justify-between items-center mb-10 gap-2">
        <h1 className="text-xl text-black font-medium">User Overview</h1>
